...
const Login: React.FC = () => {
    ...
    const handleAuth = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Verificação flexível para o formato do n8n: [ { "status": "true" } ]
            const firstItem = Array.isArray(accessResult) ? accessResult[0] : accessResult;
            const statusValue = firstItem?.status;

            const isAuthorized = statusValue === true || statusValue === 'true';

            if (!isAuthorized) {
                throw new Error('Acesso não autorizado. Verifique sua assinatura ou entre em contato com o suporte.');
            }

            // 2. Se autorizado, prosseguir com login no Supabase
                ...
                <div className="relative z-10 p-12 max-w-3xl flex flex-col justify-center h-full pt-12">
                    <div className="mb-6">
                        <h1 className="text-5xl font-bold text-white mb-2 tracking-tight leading-tight">
                            Escale suas prospecções <br />
                            no <span className="text-[#FFD700]">Piloto Automático.</span>
                        </h1>
                        <p className="text-slate-400 text-xl leading-relaxed">
                            Extraia leads, aqueça seus números, valide seus disparos e realize suas prospecções com a única plataforma de inteligência comercial e CRM.
                        </p>
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex items-start gap-3 p-2.5 rounded-2xl hover:bg-white/5 transition-all duration-300 border border-transparent hover:border-white/10 group cursor-default">
                            ...
                            <div>
                                <h3 className="text-white font-bold text-lg mb-0.5 group-hover:text-[#FFD700] transition-colors">Prospecção Multicanal Ativa</h3>
                                <p className="text-slate-400 text-sm leading-relaxed max-w-sm">
                                    Encontre os melhores leads via Google Maps, base de CNPJ e extração de grupos em segundos.
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-start gap-3 p-2.5 rounded-2xl hover:bg-white/5 transition-all duration-300 border border-transparent hover:border-white/10 group cursor-default">
                            ...
                            <div>
                                <h3 className="text-white font-bold text-lg mb-0.5 group-hover:text-[#FFD700] transition-colors">Atendimento com IA no WhatsApp</h3>
                                <p className="text-slate-400 text-sm leading-relaxed max-w-sm">
                                    Seus próprios agentes inteligentes vendendo e qualificando leads 24h por dia, sem intervenção humana.
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-start gap-3 p-2.5 rounded-2xl hover:bg-white/5 transition-all duration-300 border border-transparent hover:border-white/10 group cursor-default">
                            ...
                            <div>
                                <h3 className="text-white font-bold text-lg mb-0.5 group-hover:text-[#FFD700] transition-colors">Gestão 360º do Negócio</h3>
                                <p className="text-slate-400 text-sm leading-relaxed max-w-sm">
                                    Do disparo em massa ao fluxo de caixa. Acompanhe todas as métricas e o CRM da sua operação em tempo real.
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="w-full max-w-[400px] animate-in fade-in slide-in-from-bottom-8 duration-700 delay-200 flex flex-col items-center"> {/* Centering container content */}
                    <div className="mb-6 text-center w-full"> {/* Force text center */}
                        ...
                        <p className="text-slate-500 text-base font-medium">
                            {isForgotPassword ? 'Digite seu email para receber o link de recuperação.' : 'Por favor, insira seus dados para entrar.'}
                        </p>
                    </div>
                    ...
                    <form onSubmit={isForgotPassword ? handleForgotPassword : handleAuth} className="space-y-4 w-full"> {/* Ensure form takes full width */}
                        ...
                        >
                            {loading ? (
                                ...
                                <>
                                    <span>{isForgotPassword ? 'Enviar link de recuperação' : 'Entrar na Plataforma'}</span>
